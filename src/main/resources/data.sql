...
VALUES
    ...
    (1, DATEADD('DAY', -7, CURRENT_DATE), 1, 1),
    (1, DATEADD('DAY', -6, CURRENT_DATE), 2, 1),
    (1, DATEADD('DAY', -5, CURRENT_DATE), 3, 1),
    (1, DATEADD('DAY', -4, CURRENT_DATE), 4, 1),
    (1, DATEADD('DAY', -3, CURRENT_DATE), 1, 1),

    -- 2위: 박물관 침입 (4건) - 최근 4~7일 전 데이터
    (1, DATEADD('DAY', -7, CURRENT_DATE), 1, 2),
    (1, DATEADD('DAY', -6, CURRENT_DATE), 2, 2),
    (1, DATEADD('DAY', -5, CURRENT_DATE), 3, 2),
    (1, DATEADD('DAY', -4, CURRENT_DATE), 4, 2),

    -- 3위: 셜록의 서재 (3건)
    (1, DATEADD('DAY', -3, CURRENT_DATE), 1, 3),
    (1, DATEADD('DAY', -2, CURRENT_DATE), 2, 3),
    (1, DATEADD('DAY', -2, CURRENT_DATE), 3, 3),

    -- 4위: 서부의 무법자 (2건)
    (1, DATEADD('DAY', -7, CURRENT_DATE), 1, 4),
    (1, DATEADD('DAY', -6, CURRENT_DATE), 2, 4),

    -- 5위: 심해의 비밀 (2건)
    (1, DATEADD('DAY', -5, CURRENT_DATE), 1, 5),
    (1, DATEADD('DAY', -4, CURRENT_DATE), 2, 5),

    -- 6위: 우주선 탈출 (2건)
    (1, DATEADD('DAY', -3, CURRENT_DATE), 1, 6),
    (1, DATEADD('DAY', -2, CURRENT_DATE), 2, 6),

    -- 7위: 감옥 탈출 (1건)
    (1, DATEADD('DAY', -2, CURRENT_DATE), 1, 7),

    -- 8위: 마법사의 방 (1건)
    (1, DATEADD('DAY', -7, CURRENT_DATE), 2, 8),

    -- 예외 케이스: 집계 범위 밖 (너무 과거이거나 오늘/미래 데이터)
    -- 15일 전
    (1, DATEADD('DAY', -15, CURRENT_DATE), 1, 11),
    -- 오늘
    (1, CURRENT_DATE, 2, 11);
